...
class CrossLinked:
    def __init__(self, search_engine, target, timeout, conn_timeout=3, proxies=[], jitter=0, additional=""):
        self.results = []
        ...
        self.target = f'"{target}"' if len(additional) == 0 else f'"{target}"' + '+' + additional.replace(' ', '+') 
        self.jitter = jitter
